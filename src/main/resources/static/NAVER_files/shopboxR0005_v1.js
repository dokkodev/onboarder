...
shopadbox.status = jindo.$Class({
	$static:{
		...
		tabMenu:['productAd','mallAd','menAd']		
	}
});
/********************
 * 쇼핑박스 Main script 
...
(function initShopAdBox(){
	...
	jindo.$Element("shop_wrap").delegate("click",".st_gift",function(vEvent){clickTab(vEvent.element,'productAd','contents'); return false;});
	jindo.$Element("shop_wrap").delegate("click",".st_mens",function(vEvent){clickTab(vEvent.element,'menAd','contents'); return false;});
	jindo.$Element("shop_wrap").delegate("click",".st_shopping",function(vEvent){clickTab(vEvent.element,'mallAd','contents'); return false;});
